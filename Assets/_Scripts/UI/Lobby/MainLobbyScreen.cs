...
public class MainLobbyScreen : MonoBehaviour {
    ...
    private List<LobbyRoomPanel> _currentLobbySpawns = new();
    ...
    private void OnEnable() {
        if (_currentLobbySpawns == null) {
            _currentLobbySpawns = new();
        }
    ...
    private async void FetchLobbies() {
        try {
            ...
            var lobbyIds = allLobbies.Where(l => l.HostId != AuthService.PlayerId).Select(l => l.Id);
            ...
            if (_noLobbiesText != null) _noLobbiesText.SetActive(!_currentLobbySpawns.Any());
        }
        catch (Exception e) {
            Debug.LogError(e);
        }
